...
export default function CodePreview(props) {
  ...
  const code = useMemo(() => {
    const cprops = Object.keys(currProps).map((name) => ({
      name,
      val: currProps[name]
    }));
    return `import {${component}} from '@mi/yrn-components/Animations';\n<${component} ${cprops.reduce(
      (pre, item, index) => {
        const val = Array.isArray(item.val)
          ? "[" + item.val.toString() + "]"
